...
const LayoutSuggestions = (props: LayoutSuggestionsProps) => {
  ...
  return (
    <div className="mt-12 w-full max-w-6xl mx-auto p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          <h3 className="text-lg font-semibold mb-3 text-gray-700 dark:text-gray-200">Most Frequently Used Keys</h3>
          <ul className="space-y-2">
            {topUsedKeys.map((item) => (
              <li key={item.key} className="flex justify-between items-center py-1 px-2 rounded bg-gray-50 dark:bg-gray-700">
                <span className="font-mono text-primary dark:text-primary-light">{item.key}</span>
                <span className="text-gray-600 dark:text-gray-300">{item.frequency} hits</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          <h3 className="text-lg font-semibold mb-3 text-gray-700 dark:text-gray-200">Least Used Keys</h3>
          <ul className="space-y-2">
            {leastUsedKeys.map((item) => (
              <li key={item.key} className="flex justify-between items-center py-1 px-2 rounded bg-gray-50 dark:bg-gray-700">
                <span className="font-mono text-gray-600 dark:text-gray-400">{item.key}</span>
                <span className="text-gray-600 dark:text-gray-300">{item.frequency} hits</span>
              </li>
            ))}
          </ul>
      ...
      <div className="mt-8">
        <h3 className="text-lg font-semibold mb-3 text-gray-700 dark:text-gray-200">Home Row Efficiency</h3>
        <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-4">
          <p className="mb-2 text-gray-700 dark:text-gray-300">
            Home row usage: <span className="font-semibold">{homeRowPercentage}%</span> of all keystrokes
          </p>
          ...
          <p className="mt-4 text-sm text-gray-600 dark:text-gray-400">
            Ideally, your most frequently used keys should be on the home row (ASDFGHJKL;) for optimal typing efficiency.
          </p>
        </div>
      </div>
    </div>
